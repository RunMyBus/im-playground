...
export default function Watercalculator() {
  ...
  useEffect(() => {
    const getStateList = async () => {
      try {
        const res = await fetch(`${apiRoute}/statelist`, {
          method: "POST",
        ...
        const defaultStateId = data.Data[0]?.state_id;
  ...
  useEffect(() => {
    ...
    const getDistrictListByState = async () => {
      try {
        const res = await fetch(`${apiRoute}/districtlist`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        setDistrictid(firstDistrict?._id);
        setRainfall(firstDistrict?.rainfall);
        formik.setFieldValue("rainfall", firstDistrict?.rainfall);
      } catch (error) {
